...
export function useGetMyRestaurantOrders() {
  const { getAccessTokenSilently } = useAuth0();
  async function getMyRestaurantOrderRestaurant(): Promise<Order[]> {
    const accessToken = await getAccessTokenSilently();
  ...
  const { data: restaurantOrders, isLoading } = useQuery(
    ...
    getMyRestaurantOrderRestaurant
  );

  return { restaurantOrders, isLoading };
}

type UpdtaeOrderStatusRequest = {
  orderId: string;
  status: string;
};
...
export function useUpdateMyRestaurantOrder() {
  ...
  async function updateMyRestaurantOrder(
    updateStatusOrderRequest: UpdtaeOrderStatusRequest
  ) {
    const accessToken = await getAccessTokenSilently();
